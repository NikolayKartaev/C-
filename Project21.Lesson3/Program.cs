...
void SelectionSortMax(int[] arr)
{
    for (int i = 0; i < arr.Length - 1; i++)
    {
        ...
        int temp = arr[i]; // во временную переменную помещаем значение элемента с индексом i
        arr[i] = arr[maxPos]; // на место элемента с индексом i ставим минимальный элемент
        arr[maxPos] = temp; // на место найденного минимального элемента ставим элемент с индексом i из неотсортированного множества
    }
}
PrintArray(array);
Console.WriteLine();
